...
// Chuẩn hoá path ảnh để hiển thị
$image_path = !empty($tour['main_image']) 
                ? '/' . ltrim($tour['main_image'], '/')
                : null;
?>
...
<div class="container">
    ...
    <form method="post" action="index.php?action=editTour" enctype="multipart/form-data">
            ...
            <select class="form-select" id="staff_id" name="staff_id" required>
                ...
                if (!empty($staff_list)) {
                    foreach ($staff_list as $staff) {
                        $selected = ($staff['id'] == $tour['staff_id']) ? 'selected' : '';
                        echo '<option value="' . htmlspecialchars($staff['id']) . '" ' . $selected . '>' 
                             . htmlspecialchars($staff['name']) . '</option>';
                    }
                }
                ?>
            </select>
            ...
            <select class="form-select" id="supplier_id" name="supplier_id" required>
                ...
                if (!empty($supplier_list)) {
                    foreach ($supplier_list as $supplier) {
                        $selected = ($supplier['id'] == $tour['supplier_id']) ? 'selected' : '';
                        echo '<option value="' . htmlspecialchars($supplier['id']) . '" ' . $selected . '>' 
        ...
        <div class="mb-3">
            ...
            <?php if ($image_path): ?>
                <div class="mt-2">
                    <small class="text-muted">Ảnh cũ:</small><br>
                    <img src="<?php echo htmlspecialchars($image_path); ?>" 
                         style="width: 80px; height: auto; border:1px solid #ccc;" alt="Ảnh cũ">
                </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="date" class="form-control" id="start_date" name="start_date" 
                       value="<?php echo htmlspecialchars($tour['start_date']); ?>">
            </div>
            <div class="col-md-6 mb-3">
                <label for="end_date" class="form-label">Ngày Kết thúc:</label>
                <input type="date" class="form-control" id="end_date" name="end_date" 
                       value="<?php echo htmlspecialchars($tour['end_date']); ?>">
            </div>
        </div>
